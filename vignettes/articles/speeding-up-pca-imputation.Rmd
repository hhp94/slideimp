...
If `solver = "lobpcg"` is not clearly faster on the representative subset, force the exact solver with `solver = "exact"`.
...
`slide_imp()` typically operates on small windows with few samples, so the `"exact"` solver is usually the better default.
...
* If PCA imputation is slow, benchmark `solver = "exact"` against `solver = "lobpcg"` on a representative subset.
