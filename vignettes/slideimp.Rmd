...
We proceed to measure the accuracy of the imputation using "good" default parameters for K-NN (`k = 10`) and PCA (`ncp = 5`). For PCA imputation, we leverage the `{mirai}` package for parallelization while the `cores` argument control parallelization for K-NN (with OpenMP only).
```{r}
tune_knn <- tune_imp(
  obj,
  ...
  .f = "knn_imp",
  rep = na_location,
  cores = 4 # works for K-NN/slide_imp K-NN with OpenMP only
)

# mirai requires the carrier package for parallelization
tune_pca <- tune_imp(
  obj,
  parameters = data.frame(ncp = 5),
  .f = "pca_imp",
  rep = na_location
)
```
...
Some DNA methylation analyses only focus on epigenetic clocks, which require only a subset of CpGs to be imputed. The `knn_imp()` function takes advantage of this to significantly reduce run time. To impute only specific CpGs for K-NN imputation and make use of grouped imputation we can use the `prep_groups()` function.

Here, we simulate data where only a subset of CpGs are needed for imputation.

```{r}
# 500 features, 50 samples, 2 chromosomes
sim_obj <- sim_mat(n = 50, p = 500, n_col_groups = 2)
...
- `group_imp()` requires a `group` tibble, *preferably* created with `prep_groups()`, with three list-columns:
...
group_df <- prep_groups(colnames(obj), sim_obj$col_group, subset = subset_cpgs)
group_df
```

For demonstration purposes, we use the `parameters` column to add group-specific parameters. This can be useful if hyperparameters are tuned separately for each group, though this is typically not necessary. When `k` or `ncp` is too large for some groups, we can use the `parameters` column to specify group-specific `k` or `ncp` values. `k` and `ncp` can also be specified in `prep_groups()`.
```{r}
pca_group <- group_df
# For PCA imputation, we use ncp = 5 for the first group and ncp = 10 for the second group
pca_group$parameters <- list(list(ncp = 5), list(ncp = 10))
# For K-NN imputation, we use k = 5 for both groups
knn_group <- group_df
knn_group$parameters <- list(list(k = 5), list(k = 5))
...
We also need the `location` object, which is the actual location of each of the column (e.g., actual genomic locations). This object is used to determine which columns are grouped together given a window size. For demonstration, we are saying these columns are just 1 base pair apart.
```{r}
location <- 1:ncol(beta_matrix)
location[1:5]
```

This example dataset is small, but in actual analyses with millions of CpGs, we can tune the K-NN or PCA imputation on just the first tens of thousands of CpGs. Here, we perform K-NN imputation by specifying `k`, a window size (`window_size`) of 100 (base pairs), and a 10-bp overlap (`overlap_size`) between windows. We need to specify the minimum number of CpGs in a group that will be imputed `min_window_n`. Groups with size below `min_window_n` is not imputed.
```{r}
# For example, we are tuning `k` value here.
slide_knn_params <- data.frame(
  k = c(10, 20),
  window_size = 100,
  overlap_size = 10,
  min_window_n = 30 # has to be larger than k
)
