...
* Simulate some data
```{r}
# 10 rows, 1000 columns, all columns have at least some NA
sim_obj <- sim_mat(n = 10, p = 1000, perc_col_na = 1)
obj <- sim_obj$input
obj[1:4, 1:4]
```
...
* Here, we randomly select 5 samples from each of 200 random features for 5 repetitions using `sample_na_loc()`. To specify just certain columns (i.e., clock CpGs), provide the `na_col_subset` argument.
  - `na_loc` has 5 elements (5 repeats) where each row (row and col index) is the position of a missing value.
```{r}
...
* Using 2 repetitions of cross-validation (increase to 10-30 in real analyses). We are selecting between:
