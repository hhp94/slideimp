...
```{r}
tune_knn <- tune_imp(obj, parameters = data.frame(k = 10), .f = "knn_imp", rep = na_location, cores = 4)
tune_pca <- tune_imp(obj, parameters = data.frame(ncp = 5), .f = "pca_imp", rep = na_location)
```
...
This example dataset is small, but in actual analyses with millions of CpGs, we can tune the K-NN or PCA imputation on just the first tens of thousands of CpGs. Here, we perform K-NN imputation by specifying `k`, a window size (`window_size`) of 100, and a 10-CpG overlap (`overlap_size`) between windows.
```{r}
# For example, we are tuning `k` value here.
slide_knn_params <- tibble::tibble(k = c(10, 20), window_size = 100, overlap_size = 10)
...
```{r}
imputed_beta <- slide_imp(obj = beta_matrix, window_size = 100, overlap_size = 10, k = 10, cores = 4)
imputed_beta
```
