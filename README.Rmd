...
PCA imputation on beta values may produce values outside of the (0, 1) range. The resulting matrix can either be clamped to the (0, 1) interval afterwards, or M-values can be used for imputation.  

To use M-values, first convert the beta values to M-values with `qlogis_clamped()`, perform the imputation, and then convert the result back to beta values with `plogis()`:
