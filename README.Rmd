...
You can install the development version of `{slide_imp}` from [GitHub](https://github.com/hhp94/slide_imp) with:
...
`slide_imp()` supports passing a `big.matrix` object or the path to its descriptor file. Specify `output` for the result to change the backend to using file-backed big.matrix as well to minimize memory at a cost of performance. 
...
For more details, see the function documentation (e.g., `?slide_imp`, `?group_knn_imp`, `?knn_imp`, `?tune_imp`).
