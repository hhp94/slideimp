...
`{slideimp}` is a lightweight R package for fast k-NN and PCA imputation of missing values in high-dimensional numeric matrices (e.g., intensive longitudinal or epigenetic data).

**Core functions**  

* `knn_imp()`: full-matrix k-NN with multi-core parallelization, `{mlpack}` tree acceleration (KD/Ball-Tree, best when <20% missing per feature), and optional subset imputation (ideal for epigenetic clock calculations).
...
* `slide_imp()`: sliding-window k-NN or PCA imputation of extremely high dimensional numeric matrices with ordered features (by time or genomic position).
* `group_imp()`: group-wise (e.g. by chromosomes, or column clusters) k-NN or PCA imputation with optional auxiliary features for small groups.
* `tune_imp()`: hyperparameter tuning with cross-validation; works with built-in or custom imputation functions.
...
`knn_imp()` yields the same results as [`impute::impute.knn()`](https://bioconductor.org/packages/release/bioc/html/impute.html) without post imputation. `knn_imp()` is faster in larger data and is as fast in smaller data given `cores = 1`. `knn_imp()` scales well with multiple `cores`.
```{r}
library(bench)
...
Similarly, `pca_imp()` is an optimized version of [`missMDA::imputePCA()`](http://factominer.free.fr/missMDA/PCA.html) for high-dimensional numeric matrix.
```{r}
bench::mark(
...
Use k-NN by specifying `k`, PCA by specifying `ncp`. Only arguments applicable to each method are applied.
```{r}
...
Use `tune_imp()` to tune hyperparameters by cross-validation. This function repeatedly injects `NA` and evaluates imputation accuracy. This function supports built-in methods or custom functions. Available built-in methods are `slide_imp`, `knn_imp`, and `pca_imp`.
...
To tune a custom imputation function, a function has to take a numeric matrix `obj` as the first argument, return a matrix with the same dimension as `obj`, and parameters as arguments.
```{r}
