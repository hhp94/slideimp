...
For very large matrices that don't fit in memory, use `{bigmemory}` to create file-backed objects. `SlideKnn` supports passing a `big.matrix` object or the path to its descriptor file. Always specify `output` for the result (also file-backed). <b>NOTE</b>: see `?restore_dimnames` if output's dimnames are stripped.
