...
- **PCA performance tuning:** for guidance on choosing `threshold`, `solver`, configuring `lobpcg_control`, and using `{mirai}`/BLAS threading effectively, see [Speeding up PCA imputation](articles/speeding-up-pca-imputation.html).
...
* For PCA imputation and custom functions, set up parallelization with `mirai::daemons()`. See "[Speeding up PCA imputation](articles/speeding-up-pca-imputation.html)" for additional PCA performance-tuning guidance.
